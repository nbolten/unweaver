...
class BuildingTable(FeatureTable):
    ...
    def update_buildings(self, bbunch: Iterable[BuildingTuple]) -> None:
        serialized = [(b, self.serialize_row(d)) for b, d in bbunch]
        super().update_batch(serialized)
    
    def update_building(self, key: str, d: BuildingData) -> None:
    ...
    def insert(self, b: str, ddict: Dict[str, Any]) -> None:
        self.write_feature({**ddict, self.building_key: b})
        with self.gpkg.connect() as conn:
            conn.execute(
                f"""
